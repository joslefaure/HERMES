...
#
# from lavis.utils.compression_utils import memory_compression_kmeans
...
class Blip2VicunaInstruct_MALMM(Blip2Base):
    ...
    def __init__(
        ...
        num_frames=0,
        freeze_video_qformer=True,
        freeze_frame_qformer=True,
    ):
        logging.info(f"Initial freeze {freeze_video_qformer}, {freeze_frame_qformer}")
        super().__init__()
        ...
        # Learnable local-global weights
        # self.weight_llm = nn.Parameter(torch.tensor([0.5])) # torch.tensor([0.5])
        self.weight_llm_global = nn.Parameter(torch.rand(1)) # torch.tensor([0.5])
        
        # Use VQformer?
        # TODO: video Qformer
        self.use_global = True
        self.use_video_qformer = True
        if self.use_video_qformer:
            self.llm_proj_global = nn.Linear(
                self.Qformer.config.hidden_size, self.llm_model.config.hidden_size
            )
            self.llm_proj_global.load_state_dict(self.llm_proj.state_dict())
            
            self.video_frame_position_embedding = nn.Embedding(self.memory_bank_length, self.Qformer.config.hidden_size)
            self.video_Qformer, self.video_query_tokens = self.init_video_Qformer(
                num_query_token, self.Qformer.config.hidden_size
            )

            if freeze_video_qformer:
                for name, param in self.video_Qformer.named_parameters():
                    param.requires_grad = False
                self.video_Qformer = self.video_Qformer.eval()
                self.video_Qformer.train = disabled_train
                self.video_query_tokens.requires_grad = False
                logging.info("freeze video Qformer")
            ...
            if not qformer_text_input:
                ...
                self.video_Qformer.resize_token_embeddings(len(self.tokenizer))
            ...
            if freeze_frame_qformer:
                ...
                self.frame_Qformer.train = disabled_train
                self.video_query_tokens.requires_grad = False
                logging.info("freeze frame Qformer")
    ...
    def encode_global_features(self, global_frames, pool_type="max", pool_size=20):
        video_encoder_hidden_states = torch.cat(
            global_frames, dim=1
        )  # [B, T, N, C]
        ...
        # Flatten the num_tokens and channel dimensions
        video_encoder_hidden_states = video_encoder_hidden_states.view(
            video_encoder_hidden_states.size(0),
            video_encoder_hidden_states.size(1),
            -1,
        )

        # Permute to (Batch, num_tokens * channel, num_frames)
        video_encoder_hidden_states = video_encoder_hidden_states.permute(
            0, 2, 1
        )

        # Apply max pooling along the num_frames dimension
        video_encoder_hidden_states = F.max_pool1d(
            video_encoder_hidden_states,
            kernel_size=T // pool_size,
            stride=T // pool_size,
        )
        ...
        video_encoder_hidden_states = video_encoder_hidden_states.permute(
            0, 2, 1
        ).view(B * pool_size, N, C)
        
        query_tokens = self.frame_query_tokens.expand(video_encoder_hidden_states.size(0), -1, -1)
        image_atts = torch.ones(video_encoder_hidden_states.size()[:-1], dtype=torch.long).to(video_encoder_hidden_states.device)
        ...
        position_ids = torch.arange(pool_size, dtype=torch.long, device=video_encoder_hidden_states.device)
        position_ids = position_ids.unsqueeze(0).expand(B, -1)
        frame_position_embeddings = self.video_frame_position_embedding(position_ids)
        q_hidden_state = q_hidden_state = query_output.last_hidden_state
        # print(q_hidden_state.size())
        
        frame_position_embeddings = frame_position_embeddings.unsqueeze(-2)
        frame_hidden_state = einops.rearrange(q_hidden_state, '(b t) q h -> b t q h',b=B,t=pool_size)
        frame_hidden_state = frame_position_embeddings + frame_hidden_state

        # frame attention
        frame_hidden_state = einops.rearrange(frame_hidden_state, 'b t q h -> b (t q) h',b=B,t=pool_size)
        frame_atts = torch.ones(frame_hidden_state.size()[:-1], dtype=torch.long).to(video_encoder_hidden_states.device)
        video_query_tokens = self.video_query_tokens.expand(frame_hidden_state.shape[0], -1, -1)

        frame_atts = torch.ones(frame_hidden_state.size()[:-1], dtype=torch.long).to(frame_hidden_state.device) # [B, N]
    ...
    def encode_global_features_2(self, global_frames, pool_type="max", pool_size=20):
        video_encoder_hidden_states = torch.cat(
            global_frames, dim=1
        )  # [B, T, N, C]
        ...
        video_atts = torch.ones(video_encoder_hidden_states.size()[:-1], dtype=torch.long).to(video_encoder_hidden_states.device) # [B, N*pool_size]
    ...
    def forward(self, samples):
        ...
        if self.qformer_text_input:
            ...
            if self.use_memory_bank:
                ...
                for t in range(T):
                    ...
                    # Append after pe
                    global_frames.append(image_embeds.unsqueeze(1))
                    ...
                    if t == T - 1:
                        ...
                        # self.visual_memory_bank = self.visual_memory_bank.narrow(1, 1, self.memory_bank_length) # Naive compression
        ...
        if self.use_global:
            global_frames = self.encode_global_features(global_frames, pool_type="max", pool_size=20)
            inputs_llm_global = self.llm_proj_global(global_frames)
            
            # weights = F.softmax(torch.stack([self.weight_llm, self.weight_llm_global]), dim=0)
            # inputs_llm = inputs_llm + self.weight_llm_global * inputs_llm_global
            inputs_llm = inputs_llm + inputs_llm_global * self.weight_llm_global
    ...
    def generate(
        ...
        if self.use_global:
            ...
            # weights = F.softmax(torch.stack([self.weight_llm, self.weight_llm_global]), dim=0)
            # inputs_llm = weights[0] * inputs_llm + weights[1] * inputs_llm_global
            inputs_llm = inputs_llm + inputs_llm_global * self.weight_llm_global
        ...
        with self.maybe_autocast():
            ...
            outputs = self.llm_model.generate(
                ...
                max_new_tokens=16, # TODO: Put this in config
            )
    ...
    def from_config(cls, cfg):
        ...
        freeze_video_qformer=cfg.get("freeze_video_qformer", True)
        freeze_frame_qformer=cfg.get("freeze_frame_qformer", True)
        ...
        model = cls(
            ...
            freeze_video_qformer=freeze_video_qformer,
            freeze_frame_qformer=freeze_frame_qformer,     
        )
...
# Learnable local-global weights
        ...
        if self.use_global:
            ...
            llama_proj_model = "open_source_layers/prerained_minigpt4_7b.pth"
            print("load minigpt4 proj weight: {}".format(llama_proj_model))
            llama_proj_weight = torch.load(llama_proj_model, map_location="cpu")
            msg = self.llm_proj_global.load_state_dict(llama_proj_weight['model'], strict=False)
            # self.llm_proj_global.load_state_dict(self.llm_proj.state_dict())
            ...
            if freeze_frame_qformer:
                ...
                self.frame_Qformer = self.frame_Qformer.eval()
                self.frame_Qformer.train = disabled_train
                self.frame_query_tokens.requires_grad = False
                logging.info("freeze frame Qformer")
